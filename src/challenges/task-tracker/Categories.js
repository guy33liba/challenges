 ...
 return (
  <div>
   ...
   <button
    ...
    style={{ marginLeft: "10px", padding: "10px 20px", fontSize: "18px" }}
   >
    Add Task
   </button>
       ...
       {categories[category].map((task, index) => (
        <div key={index}>
         {isEditing?.category === category && isEditing.index === index ? (
          ...
          <div className="deleteEditBlock" style={{ width: "250px", background: "red" }}>
           <li>{task}</li>
           <div style={{ width: "250px", display: "flex", justifyContent: "end" }}>
            <button
            ...
            <button onClick={() => startEditing(category, index, task)}>Edit</button>
           </div>
          </div>
         )}
        </div>
